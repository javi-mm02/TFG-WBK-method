...
# ============================================================
# COMPROBACIÓN DEL CRITERIO DE VALIDEZ WBK RADIAL
# ============================================================
...
def compute_validity_ratio_from_wave_number(
    ...
    Calcula el cociente adimensional asociado al criterio de validez WBK,
        ...
        R(r) =
        | 3/4 (k'(r)/k(r))^2 - 1/2 k''(r)/k(r) | / |k(r)^2|.

    En la región prohibida se aplica la misma expresión usando kappa(r).
    El criterio local requiere R(r) << 1, lejos de los puntos de giro.
    """
    dk_dr = np.gradient(wave_number, coordinate, edge_order=2)
    d2k_dr2 = np.gradient(dk_dr, coordinate, edge_order=2)

    with np.errstate(divide="ignore", invalid="ignore"):
        correction = np.abs(
            0.75 * (dk_dr / wave_number) ** 2
            - 0.5 * d2k_dr2 / wave_number
        )
...
def summarize_validity_ratio(
    ...
    if selected_ratio.size == 0:
        raise RuntimeError(
            f"No valid points were found for the validity check in {region}."
        )
...
def compute_validity_checks(
    ...
    """
    Evalúa el criterio de validez WBK en la región interna permitida y en
    la barrera, excluyendo los entornos de los puntos de giro.
    """
    mu_c2 = results["mu_c2"]
    r1 = results["r1"]
    r2 = results["r2"]
    ...
    r_allowed = np.linspace(r1, r2, n_points)
    k_allowed = allowed_wave_number(r_allowed, params, mu_c2)

    validity_rows.append(
        {
            **summarize_validity_ratio(
                coordinate=r_allowed,
                wave_number=k_allowed,
                left=r1,
                right=r2,
                region="internal_allowed_region",
                exclusion_fraction=exclusion_fraction,
            )
        }
    )

    r_barrier = np.linspace(r2, r3, n_points)
    kappa_barrier = barrier_integrand(r_barrier, params, mu_c2)

    validity_rows.append(
        {
            **summarize_validity_ratio(
                coordinate=r_barrier,
                wave_number=kappa_barrier,
                left=r2,
                right=r3,
                region="barrier_region",
                exclusion_fraction=exclusion_fraction,
            )
        }
    )
...
def save_validity_table(validity_rows: list[dict]) -> None:
    csv_path = RESULTS_DIR / "alpha_validity_check.csv"
...
def print_validity_results(validity_rows: list[dict]) -> None:
    print()
    print("Radial WBK validity criterion check")
    print("===================================")
    print(
        f"{'region':>26s} "
        f"{'median R':>14s} "
...
if __name__ == "__main__":
    ...
    validity_rows = compute_validity_checks(calibrated_params, final_results)
    print_validity_results(validity_rows)
    save_validity_table(validity_rows)
