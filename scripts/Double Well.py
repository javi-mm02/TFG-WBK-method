...
# =============================================================================
# COMPROBACIÓN DEL CRITERIO DE VALIDEZ WBK EN LA BARRERA
# =============================================================================
...
def compute_validity_ratio_from_wave_number(
    ...
    En la región prohibida se aplica la misma expresión usando kappa(q).
    El criterio local requiere R(q) << 1, lejos de los puntos de giro.
    """
...
def forbidden_wave_number(
    q,
    energy: float,
    params: DoubleWellParameters,
):
    """
    Número de onda imaginario kappa(q) en la región prohibida.
    """
    value = potential(q, params) - energy
    return np.sqrt(2.0 * params.mass * np.maximum(value, 0.0)) / params.hbar
...
def compute_validity_checks(
    ...
    """
    Evalúa el criterio de validez en la región de barrera de cada doblete.
    """
    validity_rows = []
    ...
    for row in rows:
        ...
        q_values = np.linspace(q2, q3, n_points)
        kappa_values = forbidden_wave_number(q_values, energy, params)

        summary = summarize_validity_ratio(
            coordinate=q_values,
            wave_number=kappa_values,
            left=q2,
            right=q3,
        ...
        validity_rows.append(
            {
                ...
                "q3": q3,
                "S_B": row["S_B"],
                "S_B_over_hbar": row["S_B"] / params.hbar,
                "Delta_E_num": row["Delta_E_num"],
                "Delta_E_WBK": row["Delta_E_WBK"],
                "ratio_WBK_to_num": row["ratio_WBK_to_num"],
                **summary,
            }
        )
...
def save_validity_csv(validity_rows: list[dict]) -> None:
    csv_path = RESULTS_DIR / "double_well_validity_check.csv"
    ...
    print(f"Validity table saved in: {csv_path.resolve()}")


def print_validity_summary(validity_rows: list[dict]) -> None:
    print()
    print("WBK validity and barrier-opacity checks")
    print("=======================================")
    print(
        f"{'n':>3s} "
        f"{'S_B/hbar':>12s} "
    ...
    for row in validity_rows:
        print(
            f"{row['n']:3d} "
            f"{row['S_B_over_hbar']:12.6f} "
            f"{row['median_validity_ratio']:14.6e} "
            f"{row['percentile_95_validity_ratio']:14.6e} "
            f"{row['max_validity_ratio']:14.6e} "
            f"{row['ratio_WBK_to_num']:12.6e}"
        )

    print()
    print(
        "The validity ratio is evaluated in the forbidden region, "
        "excluding the Airy neighbourhoods of q2 and q3."
    )
...
if __name__ == "__main__":
    ...
    validity_rows = compute_validity_checks(params, rows)

    print_summary(params, rows)
    print_validity_summary(validity_rows)
    save_csv(rows)
    save_latex_table(rows)
    save_validity_csv(validity_rows)
